...
public class TerminationVerifier {
  ...
  public void Analyze(TerminationAnalysisInput terminationAnalysisInput) {
    ...
    boolean satisfiable = Analyze(terminationAnalysisInput,
            graphsSat,
            graphsUnSat,
            summaries);
    ...
    if (mainDir != null) {
      ...
      DotUtils.SaveGraphToSingleFile(mainDir + "graph_main.dt", satisfiable, terminationAnalysisInput.RegionTransitionGraph);
  ...
  private boolean Analyze(TerminationAnalysisInput analysisInput, List<Graph> graphsSatOut, List<Graph> graphsUnSatOut, Map<Integer, List<Graph>> summaries) {
    ...
    Map<Integer, List<Graph>> graphs = SummarizeGraph(analysisInput.RegionTransitionGraph, analysisInput.EntryNodes, analysisInput.NestingDepth);
  ...
  private Map<Integer, List<Graph>> SummarizeGraph(final Graph g, List<RTNode> entryNodes, int nestingDepth) {
    ...
    if (nestingDepth > 1) {
      ...
      result.put(1, new ArrayList<Graph>() {{add(g);}} );
    }
  ...
  private List<List> GetAllPaths(Object nodeSrc, Graph mainGraph, Graph outerGraph, Graph nestedGraph, HashSet<Object> marked) {
    ...
    for (int i = 1; i < path.size(); i++) {
      ...
      for (Map.Entry<Node, List<Node>> entry : currTr.entrySet()) {
        ...
        for (Node node : entry.getValue()) {
          nextNodes.addAll(nextTr.get(node)); // TODO some more efficient hashing
        }
